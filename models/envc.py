...
class ENVCwAR(IntraInterCodingModelBase):
	def __init__(self, Ni=128, Mi=160, N=128, M=160, num_head=4, num_point=4):
		...
		self.P.motion_enc_ft = FeatureTransformEncoder(N, M//2, k, lmin=L, lmax=4)
		self.P.motion_dec_ft = FeatureTransformDecoder(N//2, M//2, k, lmin=L, lmax=4)
	...
	def forward_inter(self, x, ref_buffer, **kwargs):
		...
		ry_weight, ry_mean, ry_scale = self.P.ry_PM(torch.cat([ry_ar, ry_hyper], dim=1))
	...
	def cross_scale_prediction(self, fx_refs, flow):
		...
		# warping
		out = self.P.warp(src_flatten, flow_flatten, spatial_shapes, level_start_index)
	...
	def GMM(self, x, weight, mean, scale):
		...
		likelihood = [weight[i] * self.GM(x, mean[i], scale[i]) for i in range(len(weight))]
